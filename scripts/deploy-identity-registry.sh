#!/usr/bin/env bash
# scripts/deploy-identity-registry.sh
#
# Deploys the identity_registry contract to a Stellar network and
# initializes it with the provided admin address.
#
# Usage:
#   bash scripts/deploy-identity-registry.sh <NETWORK> <ADMIN_ADDRESS>
#
# Arguments:
...
#
# Examples:
#   bash scripts/deploy-identity-registry.sh testnet GABC...
#   bash scripts/deploy-identity-registry.sh standalone GABC...
#
# Requirements:
...
WASM_PATH="$CONTRACTS_DIR/target/wasm32-unknown-unknown/release/identity_registry.wasm"
...
# Deploy the contract
# ---------------------------------------------------------------------------
info "Deploying identity_registry contract..."
...
# ---------------------------------------------------------------------------
echo ""
ok "Identity Registry Contract deployed and initialized!"
info "  Contract ID : $CONTRACT_ID"
info "  Network     : $NETWORK"
info "  Admin       : $ADMIN_ADDRESS"
echo ""
info "Next steps:"
info "  - Register students: stellar contract invoke --id $CONTRACT_ID --source $ADMIN_ADDRESS --network $NETWORK -- register_student --student_id <BYTES32> --nfc_hash <BYTES32> --wallet <STUDENT_ADDRESS>"
info "  - View contract docs: docs/contracts/identity-registry.md"
